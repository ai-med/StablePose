    ...
    def sample(self,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                ...
                if isinstance(ctmp, list): ctmp = ctmp[0]
                cbs = ctmp.shape[0]
                if cbs != batch_size:
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            if append_to_context is not None:
                if features_adapter is not None: # t2i-adpter only
            ...
            if features_adapter is not None:
                assert isinstance(features_adapter, list)
                features_adapter = [torch.cat([feat] * 2) for feat in features_adapter]
            out = self.model.apply_model(x_in, t_in, c_in, features_adapter=features_adapter)
